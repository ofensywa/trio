...
void Hooks::Init()
{
	...
	g_pCvar->console_color_printf(g_Settings.config.cMenuColor, "cool cheat name");
	g_pCvar->console_printf("] ");
	g::pMsg("welcome.\n");
                                 ...
                                 HWND hDestWindowOverride,  const RGNDATA* pDirtyRegion)
{
    ...
    [pDevice]()
    {
        ...
        else
        {
			...
			std::string watermark_text = g_pEngine->IsInGame() && nci ? "cool cheat name | rtt: " +
				std::to_string(int(nci->GetAvgLatency(FLOW_INCOMING) + nci->GetAvgLatency(FLOW_OUTGOING) * 1000.f)) + "ms" : "cool cheat name";
